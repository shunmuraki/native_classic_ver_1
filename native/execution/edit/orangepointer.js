...
export const keytouch_edit_command_c = () => {
    ...
    if (edit_env.block) {
        if (get_orange_space(edit_centerd_wrapper_index).scrollLeft == Number(target_data(edit_env.block, "scroll_left_"))) {
            
            if (edit_centerd_wrapper_index.classList.contains("pointer_motion") && edit_env.block.classList.contains("deleted_pointer") == false){
                // * ポインター移動中でかつ今の orange_pointer が opac_cam ではない場合.    
                deleted_pointer_delete();
                remove_orange_pointer();
            } else if (edit_env.block.classList.contains("deleted_pointer") == true) {
