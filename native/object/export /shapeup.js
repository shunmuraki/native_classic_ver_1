export const native_shapeup = () => {
    ...
    for (let i = 0; i < sections.length; i++) {
        ...
        for (let o = objects.length - 1; o >= 0; o--) {
            ...
            if (object) {
                if (object.classList.contains("same_deletable")) {
                    let the_deletable_key = "same_id_" + target_data(object, "same_id_");
                    if (get("section_deletable_list").indexOf(the_deletable_key) == -1) {
                        ...
                        for (let l = will_deleted.length - 1; l >= 0; l--) {
                            if (! will_deleted[l].isEqualNode(object)) {
                                ...
                                if (! will_deleted[l].lastElementChild) { 
                                    ...
                                    for (let j = 0; j < del_list.length; j++) {
                                        if (del_list[j].indexOf("anim_num_") != -1) {                                        
                                            object.classList.add(del_list[j]);
                                        }
                                    }
                                }
                            }
                    ...
                    if (object) {
                        class_shift(object, object, "same_deletable", "remove");
                        class_shift(object, object, "same_id_", "remove");
                    }
                }
            }
        }
